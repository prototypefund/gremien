...
class VisitorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create visitor" do
    assert_difference('Visitor.count') do
      post visitors_url, params: { visitor: { email: @visitor.email, status: @visitor.status, token: @visitor.token } }
    end
  ...
  test "should update visitor" do
    patch visitor_url(@visitor), params: { visitor: { email: @visitor.email, status: @visitor.status, token: @visitor.token } }
