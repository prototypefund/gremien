...
class PeriodsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create period" do
    assert_difference('Period.count') do
      post periods_url, params: { period: { accounts}: @period.accounts}, name: @period.name, owner_id: @period.owner_id, status: @period.status, token: @period.token } }
    end
  ...
  test "should update period" do
    patch period_url(@period), params: { period: { accounts}: @period.accounts}, name: @period.name, owner_id: @period.owner_id, status: @period.status, token: @period.token } }
